...
void CTL_list_begin(const CTL_list *handle, CTL_list_iterator *res)
{
	res->node = handle->list->next;
	res->data = res->node->data;
	res->T_size = handle->T_size;
}

void CTL_list_end(const CTL_list *handle, CTL_list_iterator *res)
{
	res->node = handle->list;
	res->data = res->node->data;
	res->T_size = handle->T_size;
}

void CTL_list_push_back(CTL_list *handle, const void *element)
{
	CTL_list_iterator end;
	CTL_list_end(handle, &end);
	CTL_list_insert(handle, &end, element);
}

void CTL_list_push_front(CTL_list *handle, const void *element)
{
	CTL_list_iterator begin;
	CTL_list_begin(handle, &begin);
	CTL_list_insert(handle, &begin, element);
}
...
void CTL_list_merge(CTL_list *handle, CTL_list *other, bool (*compare_handler)(void *a, void *b))
{
	...
	CTL_list_begin(other, &first_2);

	CTL_list_iterator last_2;
	CTL_list_end(other, &last_2);
