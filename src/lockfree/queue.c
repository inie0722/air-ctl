...
void CTL_lockfree_queue_new(CTL_lockfree_queue *handle, size_t T_size)
{
    ...
    CTL_aba_pointer_atomic_store(&__aba_ptr_get(node)->next, CTL_aba_pointer_make(NULL), memory_order_relaxed);

    CTL_aba_pointer_atomic_store(&handle->head, node, memory_order_relaxed);
    CTL_aba_pointer_atomic_store(&handle->tail, node, memory_order_release);
}
...
void CTL_lockfree_queue_push(CTL_lockfree_queue *handle, const void *element)
{
    ...
    while (1)
    {
        ...
        if (__aba_ptr_get(tail) == __aba_ptr_get(tail2))
        {
            ...
            {
                if (CTL_aba_pointer_atomic_weak(&__aba_ptr_get(tail)->next, &next, node, memory_order_release))
                {
                    CTL_aba_pointer_atomic_strong(&handle->tail, &tail, node, memory_order_release);
            ...
            {
                CTL_aba_pointer_atomic_weak(&handle->tail, &tail, next, memory_order_release);
            }
        }
    }
}
    ...
    while (1)
    {
        ...
        if (__aba_ptr_get(head) == __aba_ptr_get(head2))
        {
            if (__aba_ptr_get(head) == __aba_ptr_get(tail))
            {
                if (__aba_ptr_get(next) == NULL)
                    continue;
                CTL_aba_pointer_atomic_strong(&handle->tail, &tail, next, memory_order_release);
            }
            else
            {
                if (__aba_ptr_get(next) == NULL)
                ...
                if (CTL_aba_pointer_atomic_weak(&handle->head, &head, next, memory_order_release))
                {
