...
void CTL_tree_map_new(CTL_tree_map *handle, CTL_key *key, const size_t T_size)
{
...
void CTL_tree_map_begin(const CTL_tree_map *handle, CTL_tree_map_iterator *ret)
{
    ret->T_size = handle->T_size;
    ret->data = handle->header->left->data;
    ret->node = handle->header->left;
}

void CTL_tree_map_end(const CTL_tree_map *handle, CTL_tree_map_iterator *ret)
{
    ret->T_size = handle->T_size;
    ret->data = NULL;
    ret->node = handle->header;
}

void CTL_tree_map_insert(CTL_tree_map *handle, void *key, void *data)
{
    ...
    {
        parent = i;
        i = (handle->key.more(i->key, key) ? i->left : i->right);
    }
    ...
    else if (handle->key.more(parent->key, key))
    {
...
void CTL_tree_map_find(const CTL_tree_map *handle, const void *key, CTL_tree_map_iterator *ret)
{
...
void CTL_tree_map_iterator_move(const CTL_tree_map_iterator *handle, const ptrdiff_t index, CTL_tree_map_iterator *ret)
{
